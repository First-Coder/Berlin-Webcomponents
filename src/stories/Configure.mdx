...
<div className="sb-container">
    ...
    fb.addBlnInput({ label: 'E-Mail', name: 'email', type: 'email' })
    .addBlnInput({ label: 'Passwort', name: 'password', type: 'password' })
    .addBlnInput({ label: 'Alter', name: 'age', type: 'number', value: '18' })
    .addButton('Speichern', () => {
    ...
    Diese Validatoren stehen direkt über die Methoden validate()/validateAsync() zur Verfügung. Inline-Fehlertexte können Sie bei Bedarf über BlnInput selbst steuern (Property error).
...
`}
    ...
    ### addBlnInput(props?)
    Fügt ein BlnInput-Feld hinzu. Sie können alle BlnInput-Properties als Props übergeben (z. B. label, name, type, placeholder, hint, error, value, required usw.).
...
{`
    ...
    - setCustomValidateFunctionAsync(type: string, func: (value: string) => Promise<string></string>): this
        ...
        - validateAsync(type: string, value: string): Promise<string></string>
            ...
            - Fügt ein BlnInput-Feld ein; alle BlnInput-Properties können gesetzt werden (z. B. label, name, type, value, hint, error, required usw.).
