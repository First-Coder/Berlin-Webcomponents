...
<div className="sb-container">
    <div className='sb-section-title'>
        ...
        Registrieren Sie Ihr neues benutzerdefiniertes Element mithilfe der Methode CustomElementRegistry.define(), indem Sie den zu definierenden Elementnamen, die Klasse oder Funktion, in der die Funktionalität angegeben ist, und optional das zu ererbende Element übergeben.
        Falls erforderlich, hängen Sie ein Shadow DOM an das benutzerdefinierte Element mithilfe der Methode Element.attachShadow(). Fügen Sie dem Shadow DOM mit regulären DOM-Methoden untergeordnete Elemente, Event-Listener usw. hinzu.
        Falls erforderlich, definieren Sie eine HTML-Vorlage mit {`<template>`}- und {`<slot>`}. Verwenden Sie erneut reguläre DOM-Methoden, um die Vorlage zu klonen und an Ihr Shadow DOM anzuhängen.
        Verwenden Sie Ihr benutzerdefiniertes Element überall auf Ihrer Seite, genau wie Sie es mit einem regulären HTML-Element tun würden.


        ## Lit-Bibliothek:
        Lit ist eine einfache JavaScript-Bibliothek von Google, die die Erstellung von schnellen, leichtgewichtigen und wiederverwendbaren Webkomponenten vereinfacht. Es bietet eine Basisklasse (LitElement), die Funktionen für reaktive Eigenschaften, deklarative Vorlagen (lit-html) und kapselte Stile bereitstellt. Lit-Komponenten funktionieren in jedem Framework oder ganz ohne Framework und ermöglichen die Erstellung von wiederverwendbaren Bausteinen für Apps und Designsysteme.
        <a href="https://lit.dev/" target="_blank">Lit-Webkomponenten</a>


        ## Wichtiger Hinweis:
        In den Codebeispielen dieser Dokumentation werden mitunter folgende Zeichen verwendet ( geschweifte Klammer auf und Backtick   und  Backtick mit geschweifte Klammer zu), da es Probleme beim markdown gab.
...
<div className='sb-section-title'>
    ...
    BlnTreeView ist ein performanter, barrierearmer Tree-View als Web Component. Er unterstützt kontrollierte Auswahl und Expansion, asynchrones Nachladen von Kindknoten, Tastaturnavigation (Roving Tabindex), Single- und Multi-Select, verschieden große Darstellungen und ein Retro-Design.
...
{`const items = [
    ...
    Hinweise:
    - Diese Komponente ist kontrolliert: selectedIds und expandedIds sollten Sie aus Ihrer App-Logik aktualisieren (z. B. in Event-Handlern).
    ...
    - Wird i. d. R. über detail.resolve innerhalb des bln-tree:load-needed Handlers indirekt genutzt; kann bei Bedarf auch direkt aufgerufen werden.
...
{` const tree = document.querySelector('bln-tree');
    ...
    - Kontrolliert bleiben: Passen Sie selectedIds und expandedIds in Event-Handlern an, um den Tree-Status zu steuern.
    - Lazy-Loading: Nutzen Sie bln-tree:load-needed und rufen Sie detail.resolve(children) mit schlanken Kindlisten auf.
    - Große Datenmengen: Nur expandierte Äste rendern lassen (Standardverhalten). Bei sehr großen Bäumen ggf. Paginierung oder serverseitige Filter ergänzen.
...
<div className="sb-container">
    ...
    Der FormBuilder ist eine schlanke Hilfsklasse, mit der Sie Formularfragmente programmatisch zusammensetzen, synchron/asynchron validieren und häufige UI-Bausteine (Buttons, Tree, Checkbox, Select, Toast etc.) bequem hinzufügen können. Das Ergebnis sind Lit-Templates (TemplateResult[]), die Sie direkt in Ihre Webkomponente oder Seite rendern können.
    ...
    - Bequeme Helfer: addBlnTree, addBlnCheckbox, addModernTree, addBlnToast, addBlnSelect, addBlnButton
    ...
    // Felder + einfache Validierung
    fb.addField('email', 'E-Mail', '')
    .addField('password', 'Passwort', '')
    .addField('number', 'Alter', '18')
    .addButton('Speichern', () => {
    console.log('Speichern geklickt');
});

    // Später im Render-Kontext (z. B. in Ihrer Lit-Komponente):
    const fields = fb.getFields(); // TemplateResult[]
    // In Ihrem Template:
    {/* ${fields} */}
    `}
    ```
    ...
    Diese Validatoren werden von addField bei den entsprechenden Typen automatisch angewendet und als Text unter dem Feld gerendert (email, number). Für password wird im Ausgangscode kein Inline-Fehler gerendert; Sie können aber eigene Logik ergänzen.
...
`}
    ...
    - validateAsync(type, value): Führt den asynchronen Validator aus. Fehlt ein Async-Validator, wird automatisch validate(type, value) (sync) verwendet. Falls der Async-Validator eine Exception wirft, wird „Validierung fehlgeschlagen“ zurückgegeben (oder der String der Exception, wenn es ein String ist).

    ## Felder hinzufügen

    ### addField(type, label, value?)
    Erzeugt ein Basisfeld mit Label und optionalem Startwert.
    - type: 'email' | 'password' | 'number' | 'text' (Default-Fall)
    - label: sichtbarer Feldtitel
    - value: Startwert (string, optional)
    ...
    .addField('email', 'E-Mail', '')
    .addField('password', 'Passwort')
    .addField('number', 'Mitarbeiteranzahl', '12')
    .addField('text', 'Notiz', '…');
    ```


    Hinweise:
    - Für 'email' und 'number' wird die Fehlermeldung direkt unter dem Feld angezeigt (basierend auf validate()).
    - Für 'password' ist eine Standardprüfung (>= 5 Zeichen) hinterlegt, die im Standardmarkup nicht automatisch als Text ausgegeben wird. Sie können dieses Verhalten erweitern, indem Sie z. B. addField um eine eigene Darstellung ergänzen oder eine eigene Feldmethode schreiben.
    ...
    Die folgenden Methoden fügen gebrauchsfertige Web-Komponenten ein. Sie reichen Props durch und können so kontrollierte Zustände und A11y-Attribute setzen. Alle Methoden pushen ein Template in die interne Liste und geben this zurück (kaskadierbar).
    ...
    ```textmate
    // TypeScript
    const fb = new FormBuilder().addField('email', 'E-Mail').addButton('OK');
...
{`
    ...
    .addField('text', 'Benutzername') // Anzeige der Fehlermeldung müssten Sie bei Bedarf selbst ergänzen
    .addField('email', 'E-Mail')
    .addField('number', 'Mitarbeiterzahl', '10')
...
{`
    ...
    - Validierungslogik zentralisieren: Registrieren Sie eigene Validatoren (sync/async) einmal und verwenden Sie sie überall.
    - Fehlertexte: Leere Strings '' bedeuten gültig; alles andere wird als Fehlermeldung interpretiert.
    - Asynchrone Validierung robust machen: Fehler/Exceptions fangen; die API gibt im Fehlerfall „Validierung fehlgeschlagen“ zurück.
    - Unveränderlichkeit: Wenn Sie Items/Props an UI-Komponenten übergeben, bevorzugen Sie neue Arrays/Objekte, um reaktive Updates zuverlässig auszulösen.
    ...
    - setCustomValidateFunctionAsync(type: string, func: (value: string) => Promise<string></string>): this
        ...
        - validateAsync(type: string, value: string): Promise<string></string>
            - Führt Async-Validierung aus; fällt zurück auf validate(), wenn kein Async-Validator vorhanden ist.
            - addField(type: 'email'|'password'|'number'|'text', label: string, value?: string): this
            - Fügt ein Eingabefeld ein; simple Inline-Fehleranzeige für 'email' und 'number'.
