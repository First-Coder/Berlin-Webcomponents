...
import type {BlnGridProps} from "./BlnGrid";   // 👈 NEU
...
class FormBuilder {
    ...
    // 👇 NEU: BlnGrid einfügen
    addBlnGrid(props: Partial<BlnGridProps> = {}, inner: TemplateResult[] = []) {
        const tpl = html`<bln-grid
      .columns=${props.columns ?? 1}
      .smColumns=${props.smColumns ?? 0}
      .mdColumns=${props.mdColumns ?? 0}
      .lgColumns=${props.lgColumns ?? 0}
      .gap=${props.gap ?? 'md'}
      .asList=${props.asList ?? false}
      aria-label=${props.ariaLabelText ?? ''}
      aria-describedby=${props.ariaDescribedById ?? ''}
      class=${props.class ?? ''}
    >
      <!-- optionaler Header-Slot -->
      ${/* Wenn jemand inner[] mit slot="header" übergibt, wird es hier gerendert */''}
      ${inner.length
            ? inner
            : html`
            ...
            <div class="border rounded-md p-3 bg-white">Item 1</div>
            <div class="border rounded-md p-3 bg-white">Item 2</div>
          `}
    </bln-grid>`;
        this.fields.push(tpl);
        return this;
